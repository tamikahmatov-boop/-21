...
SETTINGS_KEYS = [
    ...
    "FAVORITES_ENABLED", "FAVORITE_COINS",
]
...
# v25.13: избранные монеты — если FAVORITES_ENABLED=True и список не пуст,
# монитор отслеживает (запрашивает цены/сигналы/ликвидации) ТОЛЬКО монеты из
# FAVORITE_COINS, а не все бессрочные USDT-фьючерсы Bybit. Список хранится
# как list[str] (символы Bybit, например "BTCUSDT") — сохраняется и
# восстанавливается вместе с остальными настройками (см. SETTINGS_KEYS).
FAVORITES_ENABLED: bool = False
FAVORITE_COINS: list[str] = []
# v25.13: жёсткий потолок на размер списка избранного. ВАЖНО: FAVORITE_COINS
# входит в SETTINGS_KEYS и целиком уезжает одним JSON в ОДНО закреплённое
# сообщение Telegram (push_state_to_telegram_sync, см. ниже) — а Telegram
# отклоняет текст сообщения длиннее 4096 символов. Без ограничения слишком
# длинный список избранного мог бы сломать сохранение/восстановление ВСЕХ
# настроек через Telegram-бэкап (не только избранного), сильно затерявшись в
# логах. 150 тикеров (~10-12 симв. каждый в JSON) держат общий JSON заметно
# ниже лимита с большим запасом на все остальные настройки.
MAX_FAVORITE_COINS = 150
# Флаг: список монет нужно пересчитать на следующем же тике monitor(), не
# дожидаясь обычного 30-минутного цикла обновления — выставляется кнопками
# избранного, чтобы включение/выключение фильтра или изменение списка
# монет применялось сразу же, а не через полчаса.
_force_symbols_refresh: bool = False
...
def reply_keyboard():
    return {
        "keyboard": [
            ...
            # Строка 18 — v25.13: ручной ввод порога роста/падения (0.1–100%)
            ["✏️ Свой порог %"],
            # Строка 19 — v25.13: избранные монеты — просмотр/редактирование списка + вкл/выкл
            ["⭐ Список избранного", "✏️ Изменить избранное"],
            ["⭐ ВКЛ (только избранное)", "⭐ ВЫКЛ (все монеты)"],
        ],
...
_KEYBOARD_TEXTS_CACHE: Optional[set[str]] = None


def _all_keyboard_texts() -> set[str]:
    """
    v25.13: плоское множество ВСЕХ текстов кнопок клавиатуры (reply_keyboard).
    Используется, чтобы режимы ожидания ручного ввода (_awaiting_input —
    custom_pct_24h/custom_percent/favorite_coins) могли отличить "пользователь
    решил вместо ввода числа/списка нажать другую кнопку меню" от "пользователь
    прислал реальное значение" — и не пытались распарсить, например,
    "📊 Статус" как число или как тикер монеты.
    Кэшируется при первом вызове — раскладка клавиатуры не меняется в рантайме.
    """
    global _KEYBOARD_TEXTS_CACHE
    if _KEYBOARD_TEXTS_CACHE is None:
        kb = reply_keyboard()
        texts: set[str] = set()
        for row in kb["keyboard"]:
            for label in row:
                texts.add(label)
        _KEYBOARD_TEXTS_CACHE = texts
    return _KEYBOARD_TEXTS_CACHE
...
def _normalize_fav_symbol(raw: str) -> str:
    """
    Приводит введённый пользователем тикер к формату символа Bybit
    (например "btc" / "BTC/USDT" / "btcusdt" → "BTCUSDT").
    """
...
def _apply_favorites_filter(all_symbols: set[str]) -> set[str]:
    """
    v25.13: если избранное включено и список не пуст — монитор отслеживает
    (запрашивает цены/сигналы/ликвидации) ТОЛЬКО эти монеты, а не все
    бессрочные USDT-фьючерсы Bybit. Монеты из избранного, которых нет среди
    реально торгуемых на Bybit (опечатка/делистинг), просто отфильтровываются
    — без ошибки, они не попадут в результат.
    Если избранное выключено или список пуст — фильтрация не применяется,
    отслеживаются все монеты (прежнее поведение).
    """
    if not FAVORITES_ENABLED or not FAVORITE_COINS:
        return all_symbols
    fav_set = set(FAVORITE_COINS)
    return all_symbols & fav_set


def _fmt_coin_list(coins: list, limit: int = 60) -> str:
    """
    v25.13: безопасно форматирует список монет для сообщения в Telegram.
    Telegram отклоняет сообщения длиннее 4096 символов (HTTP 400) — если
    пользователь вставит очень длинный список (сотни тикеров), сообщение
    с подтверждением могло бы не дойти вообще, без единого объяснения,
    почему. Поэтому в сообщении показываются только первые `limit` монет,
    а остальные — только количеством. На саму логику отслеживания (полный
    список хранится в FAVORITE_COINS) это никак не влияет — ограничение
    касается исключительно отображения.
    """
    coins = sorted(coins)
    if len(coins) <= limit:
        return ", ".join(coins)
    shown = ", ".join(coins[:limit])
    return f"{shown} … и ещё {len(coins) - limit}"
...
async def monitor():
    ...
    global liq_symbols
    global _force_symbols_refresh

    symbols          = _apply_favorites_filter(await get_symbols())
    ...
    while True:
        ...
        try:
            ...
            if now - last_symbols_upd >= 1800 or _force_symbols_refresh:
                _force_symbols_refresh = False
                new_sym = await get_symbols()
                if new_sym:
                    symbols = _apply_favorites_filter(new_sym)
                    liq_symbols = set(symbols)   # v25.1: ресинхронизация подписок на ликвидации
                    log.info("Монеты обновлены: %d (после фильтра избранного)", len(symbols))
...
async def _handle_message_impl(msg: dict):
    ...
    global FAVORITES_ENABLED, FAVORITE_COINS, _force_symbols_refresh
    ...
    # v25.13: если вместо числа пользователь нажал ДРУГУЮ кнопку меню (или
    # прислал команду "/...") — это не ввод значения, а желание сделать
    # что-то другое. Раньше такой текст пытались распарсить как число,
    # получали ошибку и "проглатывали" нажатие кнопки. Теперь режим ожидания
    # тихо снимается, а сообщение обрабатывается ниже как обычно.
    if _awaiting_input == "custom_pct_24h" and text not in _all_keyboard_texts() and not text.startswith("/"):
        ...
        return
    _awaiting_input = None if _awaiting_input == "custom_pct_24h" else _awaiting_input

    # v25.13: ручной ввод основного порога роста/падения (0.1–100%) после
    # кнопки "✏️ Свой порог %". Та же защита от "проглатывания" нажатия
    # другой кнопки, что и у custom_pct_24h выше.
    if _awaiting_input == "custom_percent" and text not in _all_keyboard_texts() and not text.startswith("/"):
        _awaiting_input = None
        try:
            val = float(text.replace(",", ".").replace("%", "").strip())
            ...
            await send_message("❌ Введите число от 0.1 до 100, например: 3.5", chat_id)
        return
    _awaiting_input = None if _awaiting_input == "custom_percent" else _awaiting_input

    # v25.13: ручной ввод списка избранных монет после кнопки
    # "✏️ Изменить избранное". Принимает монеты через запятую/пробел/перенос
    # строки, в любом регистре, с суффиксом USDT или без него (BTC →
    # BTCUSDT). "-" / "очистить" / "нет" / "0" — очищает список. Та же
    # защита от "проглатывания" нажатия другой кнопки, что и у остальных
    # режимов ожидания ввода выше.
    if _awaiting_input == "favorite_coins" and text not in _all_keyboard_texts() and not text.startswith("/"):
        _awaiting_input = None
        raw = text.strip()
        if raw in ("-", "очистить", "нет", "0"):
            FAVORITE_COINS = []
            _force_symbols_refresh = True
            await send_message(
                "🗑 Список избранных монет очищен.\n"
                + ("⚠️ Фильтр «только избранное» сейчас ВКЛЮЧЁН, но список пуст — "
                   "монитор временно отслеживает ВСЕ монеты, пока список пуст."
                   if FAVORITES_ENABLED else
                   "Фильтр избранного выключен — монитор отслеживает все монеты."),
                chat_id,
            )
            return
        parts = [p for p in re.split(r"[,\s\n]+", raw) if p]
        coins = sorted({_normalize_fav_symbol(p) for p in parts if _normalize_fav_symbol(p)})
        if not coins:
            await send_message(
                "❌ Не удалось распознать ни одной монеты. Пришлите тикеры через запятую "
                "или пробел, например: BTC, ETH, SOLUSDT",
                chat_id,
            )
            return
        # v25.13: сверяем со списком РЕАЛЬНО торгуемых на Bybit бессрочных
        # USDT-фьючерсов — иначе опечатка (например ETHH вместо ETH) молча
        # даёт 0 отслеживаемых монет, и пользователь долго не понимает,
        # почему сигналов нет вообще.
        try:
            real_symbols = await get_symbols()
        except Exception:
            real_symbols = set()
        known   = [c for c in coins if not real_symbols or c in real_symbols]
        unknown = [c for c in coins if real_symbols and c not in real_symbols]
        if not known:
            await send_message(
                f"❌ Ни одна из присланных монет не найдена среди бессрочных USDT-фьючерсов "
                f"Bybit: <b>{_fmt_coin_list(coins)}</b>\nПроверьте написание (например BTC, ETH, "
                f"SOL) и попробуйте снова кнопкой «✏️ Изменить избранное».",
                chat_id,
            )
            return
        FAVORITE_COINS = known
        _force_symbols_refresh = True
        cap_hint = ""
        if len(FAVORITE_COINS) > MAX_FAVORITE_COINS:
            dropped = FAVORITE_COINS[MAX_FAVORITE_COINS:]
            FAVORITE_COINS = FAVORITE_COINS[:MAX_FAVORITE_COINS]
            cap_hint = (
                f"\n⚠️ Список ограничен <b>{MAX_FAVORITE_COINS}</b> монетами (лимит Telegram "
                f"на размер резервной копии настроек) — не добавлены ({len(dropped)}): "
                f"{_fmt_coin_list(dropped, limit=20)}"
            )
        status_hint = (
            "✅ Фильтр «только избранное» уже <b>включён</b> — монитор будет "
            "отслеживать только эти монеты (обновится в течение нескольких секунд)."
            if FAVORITES_ENABLED else
            "ℹ️ Фильтр «только избранное» сейчас <b>выключен</b> — список сохранён, но "
            "монитор пока отслеживает все монеты. Включите кнопкой «⭐ ВКЛ (только "
            "избранное)», чтобы применить."
        )
        unknown_hint = (
            f"\n⚠️ Не найдены на Bybit и не добавлены: <b>{_fmt_coin_list(unknown)}</b>"
            if unknown else ""
        )
        await send_message(
            f"⭐ Избранные монеты ({len(FAVORITE_COINS)}): <b>{_fmt_coin_list(FAVORITE_COINS)}</b>{unknown_hint}{cap_hint}\n\n{status_hint}",
            chat_id,
        )
        return
    _awaiting_input = None if _awaiting_input == "favorite_coins" else _awaiting_input

    if text == "⭐ Список избранного":
        coins_text = _fmt_coin_list(FAVORITE_COINS) if FAVORITE_COINS else "(пусто)"
        await send_message(
            f"⭐ <b>Текущий список избранного</b> ({len(FAVORITE_COINS)}): {coins_text}\n"
            f"Фильтр «только избранное»: {'<b>ВКЛЮЧЁН</b>' if FAVORITES_ENABLED else '<b>ВЫКЛЮЧЕН</b>'}\n\n"
            f"Чтобы изменить список — нажмите «✏️ Изменить избранное».",
            chat_id,
        )
        return

    if text == "✏️ Изменить избранное":
        _awaiting_input = "favorite_coins"
        await send_message(
            f"✏️ Пришлите список монет через запятую или пробел, например:\n"
            f"<code>BTC, ETH, SOL, DOGE</code>\n"
            f"(суффикс USDT можно не указывать — он добавится сам; новый список "
            f"полностью ЗАМЕНИТ старый)\n\n"
            f"Чтобы очистить список — отправьте «-».",
            chat_id,
        )
        return

    if text == "⭐ ВКЛ (только избранное)":
        if not FAVORITE_COINS:
            await send_message(
                "⚠️ Список избранного пуст — сначала добавьте монеты кнопкой "
                "«✏️ Изменить избранное», иначе включать нечего.",
                chat_id,
            )
            return
        ...
        _force_symbols_refresh = True
        await send_message(
            f"✅ Фильтр «только избранное» <b>включён</b>. Монитор отслеживает только "
            f"{len(FAVORITE_COINS)} монет(ы): <b>{_fmt_coin_list(FAVORITE_COINS)}</b>\n"
            f"Применится в течение нескольких секунд.",
            chat_id,
        )
        return

    if text == "⭐ ВЫКЛ (все монеты)":
        FAVORITES_ENABLED = False
        _force_symbols_refresh = True
        await send_message(
            "✅ Фильтр избранного <b>выключен</b> — монитор снова отслеживает ВСЕ "
            "бессрочные USDT-фьючерсы Bybit. Список избранного сохранён и не удалён.",
            chat_id,
        )
        return

    if text == "✏️ Свой порог %":
        _awaiting_input = "custom_percent"
        await send_message(
            f"✏️ Введите порог роста/падения для алертов, число от <b>0.1</b> до "
            f"<b>100</b> (например: 3.5).\nТекущий: <b>{current_percent}%</b>",
            chat_id,
        )
        return
    ...
    if text in ("/start", "/menu"):
        await send_message(
            ...
            f"⭐ Избранное: {'<b>ВКЛЮЧЕНО</b> (' + str(len(FAVORITE_COINS)) + ' монет)' if FAVORITES_ENABLED else '<b>ВЫКЛЮЧЕНО</b> (отслеживаются все монеты)'}\n"
            ...
            f"  ✏️ Свой порог % — ручной ввод порога роста/падения (0.1–100%)\n"
            f"  ⭐ Список избранного — посмотреть текущий список избранных монет\n"
            f"  ✏️ Изменить избранное — задать новый список монет (полностью заменяет старый)\n"
            f"  ⭐ ВКЛ (только избранное) / ⭐ ВЫКЛ (все монеты) — переключить режим: "
            f"следить только за избранными монетами или за всеми\n"
            f"  ⚙️ Настройки разворота — все параметры\n"
            f"  🔄 Полный перезапуск — перезапуск без потери данных\n"
            f"  🆘 Помощь — краткая справка\n\n"
            f"<b>Команды:</b>\n"
            f"  /set_percent 2.5 — порог алертов (%, от 0.1 до 100)\n"
    ...
    if text in ("📊 Статус", "/status"):
        ...
        await send_message(
            ...
            f"(порог {CUSTOM_PCT_24H}%)\n"
            f"⭐ Избранное: {'ВКЛЮЧЕНО (' + str(len(FAVORITE_COINS)) + ' монет)' if FAVORITES_ENABLED else 'ВЫКЛЮЧЕНО (все монеты)'}\n"
